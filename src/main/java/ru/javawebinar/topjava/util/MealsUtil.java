...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealWithExceed> getFilteredWithExceeded(Collection<Meal> meals, LocalDateTime from, LocalDateTime to, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesSumByDate = meals.stream()
                .filter(meal -> DateTimeUtil.isBetween(meal.getDate(), from.toLocalDate(), to.toLocalDate()))
                .collect(
                        Collectors.groupingBy(Meal::getDate, Collectors.summingInt(Meal::getCalories))
                );

        return meals.stream()
                .filter(meal -> DateTimeUtil.isBetween(meal.getTime(), from.toLocalTime(), to.toLocalTime()))
                .map(meal -> createWithExceed(meal, caloriesSumByDate.get(meal.getDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }
}